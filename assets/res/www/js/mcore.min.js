...
(function(b,c){var a="2.1.0.6";M.plugin("3rd_fingerprint_basic",{auth:function(g){var e,h,d,f=b,i={};e=M.defaultSetting.create({defaultSetting:{dialog:true,progress:function(k,j){},callback:function(k,j){}},surrogateKeys:{},enumsKeys:{}});h=e.convert(g);h.progressResponse=M.response.on(function(j){var l,k;k=j.status;l=JSON.parse(j.result);h.progress.call(f,k,l,h)});h.progressResponse.progressing=true;h.response=M.response.on(function(j){var l,k;if(j.status=="FAIL"){k=j.status;l=j.message}else{k=j.status;try{l=JSON.parse(j.result)}catch(m){l=j.result}}h.progressResponse.expire();h.callback.call(f,k,l,h)});i.dialog=h.dialog==true?"Y":"N";i.progress=h.progressResponse.toString();i.callback=h.response.toString();d=M.execute("3rd.fingerprint.basic.wn2Auth",i);if(d.status!=="PROCESSING"){h.response.execute(d)}return d},check:function(h){var e,i,d,g=b,k={};if(h===c){d=M.execute("3rd.fingerprint.basic.wn2Check");return d}e=M.defaultSetting.create({defaultSetting:{callback:function(m,l){}},surrogateKeys:{},enumsKeys:{}});d=M.execute("3rd.fingerprint.basic.wn2Check");i=e.convert(h);if(typeof i.callback==="function"){var j,f;if(typeof(d.result)=="string"){d.result=JSON.parse(d.result)}f=d.status;j=d.result;i.callback.call(g,f,j,i);return M}return d},moveSetting:function(g){var e,h,d,f=b,i={};d=M.execute("3rd.fingerprint.basic.wn2MoveSetting");return d}},a)})(window);